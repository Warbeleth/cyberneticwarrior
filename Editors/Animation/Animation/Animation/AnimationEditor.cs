    ...
    public partial class AnimationEditor : Form
    {
        ...
        private void numUpDown_AnchorX_ValueChanged(object sender, EventArgs e)
        {
            if (BAddingComplete)
            {
                if (FramesListBox.SelectedIndex >= 0)
                {
                     AnimationsEditor.GetAnimations[FramesListBox.SelectedIndex].GetFrames[FramesListBox.SelectedIndex].Anchor = new Point((int)numUpDown_AnchorX.Value, (int)numUpDown_AnchorY.Value);
                }
            }
        }
